        ...
        $(document).ready(function () {
            ...
            ajaxcall({
                ...
                var options = {
                    ...
                    previewEmailListUrl: '/DotNetReport/ReportService.asmx/PreviewEmailList',
                    ...
                    runReportApiUrl: '/DotNetReport/ReportService.asmx/RunReportApi',
                    reportWizard: $("#modal-reportbuilder"),
                    lookupListUrl: '/DotNetReport/ReportService.asmx/GetLookupList',
                    getUsersAndRoles: '/DotNetReport/ReportService.asmx/GetUsersAndRoles',
                    sendPasswordSetupUrl: '/DotNetReport/ReportService.asmx/SendUserPasswordSetup',
...
<div>
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <ul class="nav nav-tabs flex-grow-1" role="tablist">
            ...
            <li role="presentation" class="nav-item">
                <a class="nav-link" href="#usersroles" aria-controls="usersroles" role="tab" data-bs-toggle="tab" data-bind="click: function() { usersRoles.onTabOpen(); }">
            ...
            <li role="presentation" class="nav-item">
                <a class="nav-link" href="#emailqueries" aria-controls="emailqueries" role="tab" data-bs-toggle="tab" data-bind="click: function() { emailQueries.load(); }">
                    <i class="fa fa-envelope-o"></i> Email Lists
                </a>
            </li>
...
<div class="fix-content" style="display: none;" data-bind="visible: true">
    <div class="tab-content">
        <div role="tabpanel" class="tab-pane" id="usersroles" data-bind="with: usersRoles">
                    ...
                    <div class="card mb-3" data-bind="css: { 'border-primary': loginMode() === 'standalone' }">
                        <div class="card-body py-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="loginMode" id="login-standalone" value="standalone" data-bind="checked: loginMode" />
                                <label class="form-check-label" for="login-standalone">
                                    <b><i class="fa fa-sign-in"></i> Standalone <span class="text-muted fw-normal">(Dotnet Report login pages)</span></b>
                                    <div class="small text-muted">Your application has no login of its own, or you want reporting behind a separate one. A NuGet package adds the pages and the sign-in.</div>
                                    <div class="small mt-1"><span class="text-muted">Works with:</span> Dotnet Report Managed</div>
                                </label>
                            </div>
                            <!-- ko if: loginMode() === 'standalone' -->
                            <div class="mt-3 ps-4">
                                <div class="alert alert-light border py-2 small mb-2" data-bind="visible: userSource() !== 'portal'">
                                    <i class="fa fa-info-circle"></i> Standalone login signs users in with a password, so it needs the <b>Dotnet Report Managed</b> source above.
                                    Application Code and SQL Query sources have no passwords to check.
                                </div>
                                <p class="text-muted small mb-1">The <code>DotNetReport.Login</code> package adds Login, Logout and Forgot Password pages to this application and signs users in against the Users tab.</p>
                                <pre class="bg-light border rounded p-3 small mb-2">dotnet add package DotNetReport.Login

// Program.cs
builder.Services.AddDotNetReportLogin();   // Identity + cookie sign-in against your Dotnet Report users
app.UseDotNetReportLogin();                // /Account/Login, /Account/Logout, /Account/ForgotPassword</pre>
                                <p class="text-muted small mb-0">Then uncomment <code>[Authorize]</code> as above. <code>GetSettings()</code> is filled in for you from the signed-in user.</p>
                            </div>
                            <!-- /ko -->
                        </div>
                    </div>
                    <div class="card mb-3" data-bind="css: { 'border-primary': loginMode() === 'sso' }">
                        <div class="card-body py-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="loginMode" id="login-sso" value="sso" data-bind="checked: loginMode" />
                                <label class="form-check-label" for="login-sso">
                                    <b><i class="fa fa-key"></i> Single sign-on</b>
                                    <div class="small text-muted">Your identity provider signs people in. No passwords are stored here.</div>
                                    <div class="small mt-1"><span class="text-muted">Works with:</span> any source</div>
                                </label>
                            </div>
                            <!-- ko if: loginMode() === 'sso' -->
                            <div class="mt-3 ps-4">
                                <p class="text-muted small mb-1">Sign in with your identity provider (Microsoft Entra ID, Okta, Google, or any OpenID Connect provider). Your provider issues the login; you map its claims:</p>
                                <pre class="bg-light border rounded p-3 small mb-2">// Program.cs
...
// GetSettings() in Controllers/DotNetReportApiController.cs
settings.UserId          = User.FindFirst("preferred_username")?.Value ?? User.Identity.Name;
settings.CurrentUserRole = User.Claims.Where(c => c.Type == "roles").Select(c => c.Value).ToList();
settings.ClientId        = User.FindFirst("ClientId")?.Value;</pre>
                                <p class="text-muted small mb-0">Then uncomment <code>[Authorize]</code> as above. Roles and tenants can still come from any source on the Users, Roles and Client Ids tabs.</p>
                            </div>
                            <!-- /ko -->
                        </div>
                    </div>
*@
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label">Always allow users to copy reports</label>
                                    <small class="text-muted d-block">
                                        Allow users with View Only access to copy Reports in the system.
                                    </small>
                                </div>
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" data-bind="checked: showScheduling">
                                    <label class="form-check-label">Hide Report/Dashboard Scheduling</label>
                                    <small class="text-muted d-block">
                                        Removes scheduling in reports and dashboard, select only if you don't want to use scheduling feature.
                                    </small>
                                </div>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex align-items-center justify-content-end gap-2">
                                    <button class="btn btn-sm btn-primary"
                                            data-bind="click: addNewColumn, visible: false">
                                    ...
                                    <button class="btn btn-sm btn-outline-secondary" data-bind="click: autoSort, visible: Selected">Auto Sort</button>
                                ...
                                <div class="list-group-item">
                                    ...
                                    <div class="checkbox" data-bind="if: !$parent.DynamicColumns()">
                                        <label>
                                            ...
                                            <label class="badge text-bg-warning" data-bind="visible: isNew">New</label>
                                        </label>
        ...
        <div role="tabpanel" class="tab-pane" id="emailqueries" data-bind="with: emailQueries">
            <div class="d-flex align-items-center mb-2">
                <button class="btn btn-primary btn-sm" data-bind="click: addQuery"><i class="fa fa-plus"></i> Add Email List</button>
                <span class="ms-3 text-muted small">Email lists for scheduled reports. Pick one when you schedule a report.</span>
            </div>
            <div class="alert alert-info py-2 small" data-bind="visible: queries().length === 0">No email lists yet.</div>
            <table class="table table-striped table-sm" data-bind="visible: queries().length > 0">
                <thead>
                    <tr>
                ...
                <tbody data-bind="foreach: queries">
                    <tr>
                        ...
                        <td><span data-bind="text: clientId ? clientId : 'All Clients'"></span></td>
                        <td><code class="small text-truncate d-inline-block" style="max-width: 320px;" data-bind="text: sqlQuery"></code></td>
                        <td class="text-end">
                            <button class="btn btn-sm btn-light" title="Show the emails this list returns" data-bind="click: function() { $parent.previewQuery($data); }"><i class="fa fa-search"></i></button>
                            <button class="btn btn-sm btn-light" title="Edit" data-bind="click: function() { $parent.editQuery($data); }"><i class="fa fa-pencil"></i></button>
                            <button class="btn btn-sm btn-light text-danger" title="Delete" data-bind="click: function() { $parent.deleteQuery($data); }"><i class="fa fa-trash"></i></button>
                        </td>
                    </tr>
                </tbody>
            </table>
                        ...
                        <table class="table table-sm align-middle mb-0 scheduled-reports-table">
                            ...
                            <tbody class="border-top" data-bind="foreach: scheduleRows">
                                <tr>
                                    ...
                                    <td class="text-break" data-bind="text: $root.emailQueries.emailToDisplay(item)"></td>
...
<div class="modal" id="email-query-modal" role="dialog" tabindex="-1" data-bind="with: emailQueries">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" data-bind="text: current().id() > 0 ? 'Edit Email List' : 'Add Email List'"></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body needs-validation" data-bind="with: current">
                <div class="mb-3 form-group">
                    <label class="form-label">Name</label>
                    <input type="text" class="form-control" name="emailQueryName" data-bind="textInput: name" placeholder="e.g. Site Managers" required />
                    <div class="invalid-feedback">Name is required.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Description</label>
                    <input type="text" class="form-control" data-bind="value: description" placeholder="Optional" />
                </div>
                <div class="mb-3">
                    <label class="form-label">Client Id</label>
                    <input type="text" class="form-control" data-bind="value: clientId" placeholder="Leave blank to make this available to all clients" />
                </div>
                <div class="mb-2 form-group">
                    <label class="form-label">SQL Query</label>
                    <textarea class="form-control" rows="8" style="font-family: monospace;" name="emailQuerySql" data-bind="textInput: sqlQuery" placeholder="SELECT Email FROM Users WHERE IsActive = 1" required></textarea>
                    <div class="invalid-feedback">SQL Query is required.</div>
                    <div class="form-text">Read only SELECT returning one row per email, with an <code>Email</code> column.</div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light btn-sm me-auto" data-bind="click: previewCurrentQuery"
                        title="Run the query above and show the emails it returns">
                    <i class="fa fa-search"></i> Preview Emails
                </button>
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary btn-sm" data-bind="click: saveQuery">Save</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal" id="users-roles-sql-modal" role="dialog" tabindex="-1" data-bind="with: usersRoles">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><span data-bind="text: currentSql().savedId() > 0 ? 'Edit' : 'Add'"></span> <span data-bind="text: currentSql().title"></span></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body needs-validation" data-bind="with: currentSql">
                <div class="mb-2 form-group">
                    <label class="form-label">SQL Query</label>
                    <textarea class="form-control" rows="10" style="font-family: monospace;" name="usersRolesSql"
                        data-bind="textInput: draft, attr: { placeholder: placeholder }" required></textarea>
                    <div class="invalid-feedback">SQL Query is required.</div>
                    <div class="form-text">Read only SELECT. <span data-bind="text: hint"></span></div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light btn-sm me-auto" data-bind="click: previewCurrentSql"
                    title="Run the query above and show the rows it returns">
                    <i class="fa fa-search"></i> Preview
                </button>
                <button type="button" class="btn btn-light btn-sm" data-bind="click: beautifyCurrentSql" title="Format the SQL above">
                    <i class="fa fa-magic"></i> Beautify Sql
                </button>
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary btn-sm" data-bind="click: saveSqlSource">Save</button>
            </div>
        </div>
    </div>
</div>
