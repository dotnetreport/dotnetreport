...
<div class="container-fluid">
        ...
        <div class="card folder-panel" data-bind="visible: true" style="display: none;">
            <div class="card-header">
                <nav class="navbar navbar-expand-lg navbar-light bg-light">
                    <a class="navbar-brand" href="#" data-bind="click: function() {SelectedFolder(null); designingHeader(false); designingFooter(false); searchReports(''); $('#search-report').val([]).trigger('change'); }">Manage Reports</a>
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav mr-auto">
                            <li class="nav-item" data-bind="visible: (CanSaveReports() || adminMode()) && (!SelectedFolder() || SelectedFolder().canEdit || adminMode())">
                            ...
                            <li class="nav-item dropdown" data-bind="visible: (CanManageFolders() || adminMode()) && SelectedFolder()!=null && (SelectedFolder().canEdit || SelectedFolder().canDelete || adminMode())">
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    <a class="dropdown-item" href="#"
                                       data-bind="click: ManageFolder.addSubFolder, visible: SelectedFolder() && (SelectedFolder().canEdit || adminMode())">
                                        <span class="fa fa-folder"></span> Add Sub Folder
                                    </a>
                                </div>
                            </li>
                            <li class="nav-item dropdown" data-bind="visible: adminMode() && SelectedFolder() ==null">
                                <a href="#" class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <span class="fa fa-window-maximize"></span> Header / Footer
                                </a>
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" href="#" data-bind="click: function(){ initHeaderDesigner(false); }">
                                        <span class="fa fa-arrow-up"></span> Report Header
                                    </a>
                                    <a class="dropdown-item" href="#" data-bind="click: function(){ initFooterDesigner(false); }">
                                        <span class="fa fa-arrow-down"></span> Report Footer
                                    </a>
                                </div>
                            </li>
                        ...
                        <div class="form-inline my-2 my-lg-0 ms-auto w-50">
                            <div class="input-group w-100">
                                ...
                                <span data-bind="visible: !selectMode(), click: function(){ onlyFavorites(!onlyFavorites()); }, css: { 'bg-warning-subtle': onlyFavorites() }, attr: { title: onlyFavorites() ? 'Showing only Favorites - click to show all' : 'Show only Favorites' }"
                                      class="input-group-text" style="height: 38px; cursor: pointer;">
                                    <i data-bind="css: onlyFavorites() ? 'fa fa-star text-warning' : 'fa fa-star-o'"></i>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card-body">
                <div data-bind="visible: designingHeader, with: headerDesigner" style="display: none;" class="overflow-auto">
                    <div class="report-header-designer">
                        <div class="d-flex align-items-center flex-wrap mb-3">
                            <h6 class="mb-0 me-2"><i class="fa fa-window-maximize text-primary"></i> Report Headers</h6>
                            <span class="text-muted small">A client (or Global) can have several named headers. The one marked <b>Default</b> is used when a report doesn't pick one.</span>
                        </div>

                        <!-- Overall on/off for report headers -->
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="rptHeaderUse" data-bind="checked: UseReportHeader">
                            <label class="form-check-label" for="rptHeaderUse">Use report headers <span class="text-muted small">(uncheck to stop applying headers in reports)</span></label>
                        </div>

                        <!-- Off state (use `if` so .d-flex !important can't override a hidden state) -->
                        <div data-bind="if: !UseReportHeader()">
                            <div class="alert alert-secondary d-flex align-items-center justify-content-between py-2">
                                <span><i class="fa fa-eye-slash"></i> Report headers are turned off and won't be applied in reports.</span>
                                <button class="btn btn-primary btn-sm" data-bind="click: saveHtmlHeader"><i class="fa fa-save"></i> Save</button>
                            </div>
                        </div>

                        <!-- Designer (hidden when report headers are turned off) -->
                        <div data-bind="visible: UseReportHeader">
                        <!-- Selection toolbar -->
                        <div class="d-flex flex-wrap align-items-end gap-2 mb-3">
                            <div data-bind="visible: clientListIds().length > 0">
                                ...
                                <select class="form-select form-select-sm" style="min-width: 200px;"
                                        data-bind="options: clientListIds,
                                            ...
                                            optionsCaption: 'Global (all clients)',
                                            event: { change: function () { loadHeadersList(); } }">
                                </select>
                            </div>
                            <div>
                                <label class="form-label small text-muted mb-1">Header</label>
                                <select class="form-select form-select-sm" style="min-width: 240px;"
                                        data-bind="options: headersList,
                                            optionsText: 'label',
                                            optionsValue: 'id',
                                            value: selectedHeaderId,
                                            event: { change: function () { loadHeaderById(); } }">
                                </select>
                            </div>
                            <div class="ms-auto d-flex gap-2">
                                <button class="btn btn-sm btn-outline-primary" data-bind="click: newHeader"><i class="fa fa-plus"></i> New</button>
                                <button class="btn btn-sm btn-outline-danger" data-bind="click: deleteHeader, visible: selectedHeaderId() > 0"><i class="fa fa-trash"></i> Delete</button>
                            </div>
                        </div>

                        <!-- Editor card -->
                        <div class="card shadow-sm">
                            <div class="card-header bg-light py-2 d-flex align-items-center flex-wrap gap-2">
                                <div class="flex-grow-1" style="min-width: 220px;">
                                    <input class="form-control form-control-sm fw-semibold" type="text" placeholder="Header name (e.g. Invoice header)" data-bind="value: headerName">
                                </div>
                                <div class="form-check mb-0">
                                    <input class="form-check-input" type="checkbox" id="rptHeaderDefault" data-bind="checked: isDefault">
                                    <label class="form-check-label small" for="rptHeaderDefault">Default for this scope</label>
                                </div>
                            </div>
                                ...
                                <div class="d-flex flex-wrap align-items-center gap-3 mb-2">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{page.number}'); }">Page Number</a></li>
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{page.total}'); }">Total Pages</a></li>
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{current.user}'); }">Current User</a></li>
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{current.user.roles}'); }">Current User Roles</a></li>
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{current.datetime}'); }">Current Date/Time</a></li>
                                            <li><a class="dropdown-item" href="#" data-bind="click: function() { insertPlaceholder('{report.name}'); }">Report Name</a></li>
                                        </ul>
                                    </div>
                                    <div class="form-check mb-0">
                                        <input class="form-check-input" type="checkbox" id="rptHeaderEveryPage" data-bind="checked: IncludeOnEveryPage">
                                        <label class="form-check-label small" for="rptHeaderEveryPage">Repeat on every page (PDF/Word)</label>
                                    </div>
                                </div>
                                <textarea id="report-header-editor"></textarea>
                            </div>
                            <div class="card-footer bg-white d-flex align-items-center flex-wrap gap-2 py-2">
                ...
                <div data-bind="visible: designingFooter, with: footerDesigner" style="display: none;" class="overflow-auto">
                    <div class="report-header-designer">
                        <div class="d-flex align-items-center flex-wrap mb-3">
                            <h6 class="mb-0 me-2"><i class="fa fa-window-minimize text-primary"></i> Report Footer</h6>
                            <span class="text-muted small">One footer per client (or Global), applied to reports where footers are turned on.</span>
                        </div>

                        <!-- Overall on/off for report footers -->
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="rptFooterUse" data-bind="checked: UseReportFooter">
                            <label class="form-check-label" for="rptFooterUse">Use report footers <span class="text-muted small">(uncheck to stop applying footers in reports)</span></label>
                        </div>

                        <!-- Off state (use `if` so .d-flex !important can't override a hidden state) -->
                        <div data-bind="if: !UseReportFooter()">
                            <div class="alert alert-secondary d-flex align-items-center justify-content-between py-2">
                                <span><i class="fa fa-eye-slash"></i> Report footers are turned off and won't be applied in reports.</span>
                                <button class="btn btn-primary btn-sm" data-bind="click: saveHtmlFooter"><i class="fa fa-save"></i> Save</button>
                            </div>
                        </div>
                        ...
                        <div class="card shadow-sm">
                            <div class="card-body p-2">
                                <div class="d-flex flex-wrap align-items-center gap-3 mb-2">
                                    ...
                                    <div class="form-check mb-0">
                                        <input class="form-check-input" type="checkbox" id="rptFooterEveryPage" data-bind="checked: IncludeOnEveryPage">
                                        <label class="form-check-label small" for="rptFooterEveryPage">Repeat on every page (PDF/Word)</label>
                                    </div>
                                </div>
                                <textarea id="report-footer-editor"></textarea>
                            </div>
                            <div class="card-footer bg-white d-flex align-items-center flex-wrap gap-2 py-2">
                                <button type="button" class="btn btn-sm btn-link text-decoration-none px-0" data-bind="visible: !showClientScope(), click: function() { showClientScope(true); }">
                ...
                <div data-bind="visible: !designingHeader() && !designingFooter()">
                    ...
                    <div data-bind="if: layout()=='icons'">
                        <div data-bind="visible: !SelectedFolder() && !searchReports()">
                            ...
                            <ul class="folder-list" data-bind="foreach: rootFolders">
                        ...
                        <div data-bind="visible: SelectedFolder() || searchReports()">
                            <div class="clearfix">
                                ...
                                <div class="pull-right">
                                    <a href="#" class="btn btn-light" data-bind="click: goBackFolder">
                                        <span data-bind="text: SelectedFolder() && SelectedFolder().ParentFolderId ? '..back to ' + (folderBreadcrumb().length > 1 ? folderBreadcrumb()[folderBreadcrumb().length - 2].FolderName : 'Folders') : '..back to Folders'"></span>
                                    </a>
                                </div>
                            </div>
                            <div class="list-group list-overflow">
                                ...
                                <div data-bind="if: searchReports() && reportsInSearch().length==0 && foldersInSearch().length==0">
                                    No results found matching your search.
                                </div>

                                <!-- Search: matching folders -->
                                <div data-bind="if: searchReports() && foldersInSearch().length > 0">
                                    <div class="mt-1 mb-1"><strong><span class="fa fa-folder"></span> Matching Folders</strong></div>
                                    <ul class="folder-list" data-bind="foreach: foldersInSearch">
                                        <li data-bind="click: function(){ $parent.SelectedFolder($data); $parent.searchReports(''); }">
                                            <span class="fa fa-3x fa-folder text-secondary"></span>
                                ...
                                <!-- SUB-FOLDERS inside selected folder -->
                                <div data-bind="if: !searchReports() && subFoldersInFolder().length > 0">
                                    <ul class="folder-list" data-bind="foreach: subFoldersInFolder">
                                        <li data-bind="click: function(){ $parent.SelectedFolder($data); }">
                                            <span class="fa fa-3x fa-folder text-secondary"></span>
                                            <span class="desc" data-bind="text: FolderName"></span>
                                ...
                                <div class="list-group"
                                     data-bind="foreach: searchReports() ? reportsInSearch() : reportsInFolder()">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-7">
                                                ...
                                                <div class="pull-left">
                                                    <h4>
                                                        ...
                                                        <span class="fa favorite-hover-star ms-1"
                                                              data-bind="css: isFavorite() ? 'fa-star text-warning favorited' : 'fa-star-o',
                                                                         click: $parent.toggleFavorite, clickBubble: false,
                                                                         visible: !$root.selectMode()"
                                                              title="Add/Remove Favorite"></span>
                                                    </h4>
                                                </div>
                                                ...
                                                <div data-bind="if: isSubReportOnly && isSubReportOnly()">
                                                    <span class="badge bg-warning text-dark mt-1"><i class="fa fa-link"></i> Sub Report Only</span>
                                                </div>
                                            ...
                                            <div class="col-sm-5 right-align">
                                                <div class="d-none d-md-block">
                                                    <br />
                                                    <button class="btn btn-sm btn-outline-secondary" data-bind="visible: canEdit, click: openReport" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder">
                                                        <span class="fa fa-edit" aria-hidden="true"></span> Edit
                                                    </button>
                                                    <button class="btn btn-sm btn-outline-secondary" data-bind="click: copyReport, visible: (canEdit || $parent.appSettings.canCopyReport) && ($root.CanSaveReports() || $root.adminMode())" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder">
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right">
                                                            <a class="dropdown-item" href="#" data-bind="click: $parent.toggleFavorite, visible: !$root.selectMode()">
                                                                <span data-bind="css: isFavorite() ? 'fa fa-star text-warning' : 'fa fa-star-o'"></span>
                                                                <span data-bind="text: isFavorite() ? 'Remove from Favorites' : 'Add to Favorites'"></span>
                                                            </a>
                                                            <div class="dropdown-divider" data-bind="visible: !$root.selectMode()"></div>
                                                            <a class="dropdown-item" href="#" data-bind="click: function() { exportReport(ko.unwrap($root.appSettings.useAltPdf) ? 'pdfalt' : 'pdf');}">
                                                            ...
                                                            <div class="dropdown-divider" data-bind="visible: !isSubReportOnly()"></div>
                                                            <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#modal-schedule-report" data-bind="visible: !isSubReportOnly(), click: function() { $parent.openScheduleModal($data); }">
                                                                <span class="fa fa-hourglass"></span> Schedule Report
                                                            </a>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="d-block d-md-none">
                                                    <button class="btn btn-sm btn-outline-secondary" title="Edit" data-bind="visible: canEdit, click: openReport" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder">
                                                        <span class="fa fa-edit" aria-hidden="true"></span>
                                                    </button>
                                                    <button class="btn btn-sm btn-light" data-bind="click: copyReport, visible: (canEdit || $parent.appSettings.canCopyReport) && ($root.CanSaveReports() || $root.adminMode())" title="Copy Report" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder">
                    ...
                    <div data-bind="if: layout()=='list'">
                        <div class="file-explorer-container">
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb" data-bind="foreach: breadcrumbItems">
                                    <li class="breadcrumb-item" data-bind="css: { 'active': isActive }">
                                        <a href="#" data-bind="text: FolderName, click: onClick, visible: !isActive"></a>
                                        <span data-bind="text: FolderName, visible: isActive"></span>
                                    </li>
                                </ol>
                            </nav>
                            ...
                            <div data-bind="visible: !SelectedFolder() && !searchReports()">
                                <div class="container-fluid">
                                    <div class="row fw-bold border-bottom py-2 w-100">
                                        ...
                                        <div class="col-md-8" data-bind="visible: adminMode">Access</div>
                                    </div>
                                    <div class="" data-bind="foreach: rootFolders">
                                        <div class="row py-2 border-bottom align-items-center w-100 row-hover">
                                            <div class="col-md-3 d-flex align-items-center clickable" data-bind="click: function(){ $parent.SelectedFolder($data); }">
                                                <span data-bind="visible: $root.selectMode, click: function(d,e){ isSelected(!isSelected()); e.stopPropagation(); },
                                                css: isSelected() ? 'fa fa-check-square text-primary me-2' : 'fa fa-square-o me-2'"></span>
                            ...
                            <div data-bind="visible: SelectedFolder() || searchReports()">
                                <div class="container-fluid">
                                    ...
                                    <!-- FOLDERS + REPORTS (one unified list, folders first like a file explorer) -->
                                    <div data-bind="if: (!searchReports() && subFoldersInFolder().length > 0) || reportsInFolder().length > 0 || reportsInSearch().length > 0">
                                        <div class="row fw-bold border-bottom py-2 w-100">
                                            <div class="col-md-3">Name</div>
                                            <div class="col-md-1"></div>
                                            <div class="col-md-2">Modified</div>
                                            <div class="col-md-2">Modified By</div>
                                        ...
                                        <!-- Sub folder rows (same column grid as report rows) -->
                                        <div data-bind="foreach: !searchReports() ? subFoldersInFolder() : []">
                                            <div class="row border-bottom py-2 align-items-center w-100 row-hover">
                                                <div class="col-md-3 d-flex align-items-center clickable"
                                                     data-bind="click: function(){ $root.SelectedFolder($data); }">
                                                    <span class="fa fa-folder text-secondary me-2"></span>
                                                    <span data-bind="text: FolderName"></span>
                                                </div>
                                                <div class="col-md-1" onclick="event.stopPropagation()">
                                                    <div class="dropdown hover-ellipsis"
                                                         data-bind="visible: ($root.CanManageFolders() || $root.adminMode()) && (canEdit || canDelete || $root.adminMode())">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <a class="dropdown-item" href="#"
                                                                   data-bind="click: function(){ $root.SelectedFolder($data); $root.ManageFolder.editFolder(); },
                                                                    visible: canEdit || $root.adminMode()">
                                                                    <span class="fa fa-edit"></span> Edit Folder
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item text-danger" href="#"
                                                                   data-bind="click: function(){ $root.SelectedFolder($data); $root.ManageFolder.deleteFolder(); },
                                                                    visible: canDelete || $root.adminMode()">
                                                                    <span class="fa fa-trash"></span> Delete Folder
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a class="dropdown-item" href="#"
                                                                   data-bind="click: function(){ $root.ManageFolder.addSubFolder($data); },
                                                                    visible: canEdit || $root.adminMode()">
                                                                    <span class="fa fa-folder"></span> Add Sub Folder
                                                                </a>
                                                            </li>
                                        ...
                                        <div class="report-list" data-bind="foreach: searchReports() ? reportsInSearch() : reportsInFolder()">
                                            <div class="row border-bottom py-2 align-items-center w-100 row-hover">
                                                <div class="col-md-3 d-flex align-items-center">
                                                    ...
                                                    <span data-bind="text: message" class="highlight small"></span>
                                                    <span class="row-hover-actions ms-2" data-bind="css: { 'favorited': isFavorite() }, visible: !$root.selectMode()">
                                                        <a href="#" data-bind="click: $parent.toggleFavorite, attr: { title: isFavorite() ? 'Remove from Favorites' : 'Add to Favorites' }">
                                                            <i data-bind="css: isFavorite() ? 'fa fa-star text-warning' : 'fa fa-star-o'"></i>
                                                        </a>
                                                    </span>
                                                    <span class="badge bg-warning text-dark ms-1"
                                                          title="This report is only used as a linked or sub report"
                                                          data-bind="visible: isSubReportOnly && isSubReportOnly()">
                                                        <i class="fa fa-link"></i> Sub Report Only
                                                    </span>
                                                </div>
                                                    ...
                                                    <div class="dropdown hover-ellipsis">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li data-bind="visible: canEdit">
                                                                <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder" data-bind="click: openReport">
                                                            ...
                                                            <li class="dropdown-submenu">
                                                                <a class="dropdown-item dropdown-toggle" href="#"><span class="fa fa-download"></span> Export</a>
                                                                <ul class="dropdown-menu">
                                                                    <li>
                                                                        <a class="dropdown-item" href="#" data-bind="click: function() {exportReport(ko.unwrap($root.appSettings.useAltPdf) ? 'pdfalt' : 'pdf');}">
                                                                            <span class="fa fa-file-pdf-o"></span> Download PDF
                                                            ...
                                                            <li data-bind="visible: !$root.selectMode() || !isSubReportOnly()"><hr class="dropdown-divider"></li>
                                                            <li data-bind="visible: !$root.selectMode()">
                                                                <a class="dropdown-item" href="#" data-bind="click: $parent.toggleFavorite">
                                                                    <span data-bind="css: isFavorite() ? 'fa fa-star text-warning' : 'fa fa-star-o'"></span>
                                                                    <span data-bind="text: isFavorite() ? 'Remove from Favorites' : 'Add to Favorites'"></span>
                                                                </a>
                                                            </li>
                                                            <li data-bind="visible: !isSubReportOnly()">
                                                                <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#modal-schedule-report" data-bind="click: function() { $parent.openScheduleModal($data); }">
                                                                    <span class="fa fa-hourglass"></span> Schedule Report
                                                                </a>
                                                            </li>
                                                            <li data-bind="visible: $root.adminMode">
                                                                <a class="dropdown-item" href="#" data-bind="click: function() { $root.openAccessModal($data); }">
                                                ...
                                                <div class="col-md-2 small text-muted" data-bind="text: dateLastUpdated"></div>
                                                <div class="col-md-2 small text-muted text-truncate" data-bind="text: modifiedBy || '', attr: { title: modifiedBy || '' }"></div>
    ...
    <div data-bind="visible: (ReportMode() == 'execute' || ReportMode() == 'linked' || ReportMode() == 'design') && !isModalOpen()">
                ...
                <nav class="navbar navbar-expand-lg navbar-light bg-light">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <button class="btn btn-light btn-sm" data-bind="visible: canGoBackToParent(), click: backToParentReport">
                                <i class="fa fa-arrow-circle-left"></i>
                                <span>Back to Parent Report</span>
                            </button>
                            <button class="btn btn-light btn-sm" data-bind="visible: $root.CanEdit() && ReportID(), click: function(){ SaveReport(true); }" data-bs-toggle="modal" data-bs-target="#modal-reportbuilder">
                            ...
                            <div class="dropdown" data-bind="hidden: (DontExecuteOnRun() && !reportRan()) || !ReportID()">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">                                    
                                    ...
                                    <li class="dropdown-submenu dropdown-submenu-left" data-bind="visible: ko.unwrap(appSettings.useAltPdf) && canDrilldown() && !hasPivotColumn() && ReportType() != 'Single'">
                                        <a class="dropdown-item dropdown-toggle" href="#"><i class="fa fa-file-pdf-o"></i> Pdf</a>
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a href="#" class="dropdown-item" data-bind="click: appSettings.showPageSize
                                                      ? function() { PdfPage.isExpanded = false; $('#pdfOptionsModal').modal('show'); }
                                                      : function() { downloadPdfAlt(); }">Standard</a>
                                            </li>
                                            <li>
                                                <a href="#" class="dropdown-item" data-bind="click: appSettings.showPageSize
                                                      ? function() { PdfPage.isExpanded = true; $('#pdfOptionsModal').modal('show'); }
                                                      : function() { downloadPdfAltWithDrilldown('',''); }">Expanded</a>
                                            </li>
                                        </ul>
                                    </li>
                                    <!-- Alt PDF, no drilldown / pivot / kpi -> flat (no expand) -->
                                    <li data-bind="visible: ko.unwrap(appSettings.useAltPdf) && (!canDrilldown() || hasPivotColumn() || ReportType() == 'Single')">
                                        <a href="#" class="dropdown-item" data-bind="click: appSettings.showPageSize
                                              ? function() { PdfPage.isExpanded = false; $('#pdfOptionsModal').modal('show'); }
                                    ...
                                    <li class="dropdown-submenu dropdown-submenu-left" data-bind="visible: canDrilldown()">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li data-bind="visible: hasSubReports()"><a href="#" class="dropdown-item" data-bind="click: downloadExcelWithSubreport">Excel with Subreport</a></li>
                                        </ul>
                                    </li>
                                    ...
                                    <li data-bind="visible:!canDrilldown() && hasSubReports()">
                                        <a href="#" class="dropdown-item" data-bind="click: downloadExcelWithSubreport">
                                            <i class="fa fa-file-excel-o"></i> Excel with Subreport
                                        </a>
                                    </li>
                        ...
                        <div data-bind="with: $root">
                            ...
                            <div class="report-render" data-bind="css: { 'report-expanded': isExpanded }">
                                <div class="report-menubar bg-light" data-bind="hidden: DontExecuteOnRun() && !reportRan()">
                                    <div class="col-xs-12 col-centered d-flex justify-content-between flex-wrap" data-bind="with: pager">
                                        <div class="form-inline d-flex" data-bind="visible: pages()">
                                            ...
                                            <div class="pull-left">
                                                ...
                                                <div data-bind="with: $parent" class="pull-left">
                                                    ...
                                                    <div class="dropdown-content" style="position: absolute;" data-bind="visible: dropdownOpen">
                                                        <div data-bind="foreach: tableStyles">
                                ...
                                <div class="report-canvas" data-bind="hidden: DontExecuteOnRun() && !reportRan()">
                                    <div class="report-container bg-light">
                                           <div class="report-inner" data-bind="style: { backgroundColor: tableSettings().backColor || (ReportResult() && ReportResult().ReportData() && ReportResult().ReportData().BackColor ? ReportResult().ReportData().BackColor() : '#fff') }">
                                            ...
                                            <div data-bind="visible: headerDesigner.UseReportHeader">
                                                <div id="report-header" width="900" height="120" data-bind="html: $root.substituteReportPlaceholders($root.headerDesigner.headerHtml())"></div>
                                            </div>
                                            ...
                                            <div data-bind="visible: footerDesigner.UseReportFooter">
                                                <div id="report-footer" width="900" data-bind="html: $root.substituteReportPlaceholders($root.footerDesigner.footerHtml())"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<!-- Folder Edit Modal -->
<div class="modal" id="manage-access-modal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content" data-bind="with: ManageFolder">
            ...
            <div class="modal-body">
                <div class="alert alert-info py-2 mb-3"
                     data-bind="visible: IsNew() && ParentFolderId()">
                    <span class="fa fa-folder me-1"></span>
                    Creating sub folder inside:
                    <strong data-bind="text: ParentFolderName"></strong>
                </div>
                <div class="alert alert-secondary py-2 mb-3"
                     data-bind="visible:!IsNew() &&ParentFolderId()">
                    <span class="fa fa-folder-open me-1"></span>
                    Sub folder of:
                    <strong data-bind="text: ParentFolderName"></strong>
                </div>
                ...
                <div data-bind="visible: $root.adminMode" class="form-group">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="showAdminOnly" data-bind="checked: ShowAdminOnly">
                        <label class="form-check-label" for="showAdminOnly">
                            Show Admin Only (Only visible when Admin Mode is ON)
                        </label>
                    </div>
                </div>
                <br />
...
<div class="modal" id="modal-schedule-report" tabindex="-1" aria-labelledby="scheduleReportModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content" data-bind="with: scheduleReportModal">
            <div class="modal-header">
                <h5 class="modal-title" id="scheduleReportModalLabel">
                    <i class="fa fa-hourglass me-2"></i> Schedule Report
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <h6 data-bind="text: reportName" class="mb-3"></h6>

                <div class="text-center py-4" data-bind="visible: loading">
                    <span class="fa fa-spinner fa-spin fa-2x text-secondary"></span>
                    <div class="text-muted small mt-2">Loading schedules...</div>
                </div>

                <!-- all schedules for this report -->
                <div data-bind="visible: viewMode() === 'list' && !loading()">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <strong>Schedules</strong>
                        <button type="button" class="btn btn-sm btn-primary" data-bind="click: addSchedule">
                            <i class="fa fa-plus"></i> Add Schedule
                        </button>
                    </div>
                    <div class="text-muted small mb-2" data-bind="visible: schedules().length === 0">
                        No schedules yet. Click "Add Schedule" to create one.
                    </div>
                    <table class="table table-sm align-middle" data-bind="visible: schedules().length > 0">
                        <thead>
                            <tr>
                                <th>When</th>
                                <th>Email</th>
                ...
                <div data-bind="visible: viewMode() === 'filter'">
                    ...
                    <div class="small text-muted mb-2">
                        ...
                        any filter changes made later on the report won't affect this schedule. Use <b>Report's Filters</b> to remove the
                        override and follow the report's current filters again.
                    </div>
                    <div data-bind="template: { name: 'filter-group', data: $parent.scheduleFilterHost }"></div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bind="visible: viewMode() === 'list'" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-secondary" data-bind="visible: viewMode() === 'edit', click: backToList">Back</button>
                <button type="button" class="btn btn-primary" data-bind="visible: viewMode() === 'edit', click: saveSchedule">Save Schedule</button>
                <button type="button" class="btn btn-secondary" data-bind="visible: viewMode() === 'filter', click: backToList">Back</button>
                <button type="button" class="btn btn-outline-secondary" data-bind="visible: viewMode() === 'filter', click: resetScheduleFilter" title="Remove this schedule's filter override and follow the report's saved filters">Use Report's Filters</button>
                <button type="button" class="btn btn-primary" data-bind="visible: viewMode() === 'filter', click: applyScheduleFilter">Apply Filter</button>
            </div>
        </div>
    </div>
</div>
