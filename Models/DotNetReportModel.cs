...
namespace ReportBuilder.Web.Models
{
    ...
    public static class DotNetReportHelper
    {
        ...
        public static string dbtype = DbTypes.MySQL.ToString().Replace("_", " ");
                ...
                string expandSqls = null, List<ReportHeaderColumn> columns = null, bool includeSubtotal = false, bool pivot = false, string pivotColumn = null, string pivotFunction = null, List<ReportHeaderColumn> onlyAndGroupInDetailColumns = null, bool isSubReport = false)
        {
            ...
            if (!string.IsNullOrEmpty(pivotColumn))
            {
                if (!useAltPivot)
                {
                    var pd = await databaseConnection.GetPivotTable(databaseConnection, connectionString, dt, qry.sql, sqlFields, expandSqls, pivotColumn, pivotFunction, 1, int.MaxValue, null, false);
                    dt = pd.dt;
                      ...
                      string pivotColumn = null, string pivotFunction = null, bool imageOnly = false, bool debug = false, string pageSize = "", string pageOrientation = "", bool subTotalMode = false, bool includeColumnTotal = false)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(pivotColumn))
                {
                    if (!useAltPivot)
                    {
                        var pd = await databaseConnection.GetPivotTable(databaseConnection, connectionString, dt, qry.sql, sqlFields, expandSqls, pivotColumn, pivotFunction, 1, int.MaxValue, null, false, false, includeColumnTotal, subTotalMode);
                        dt = pd.dt;
                        if (!string.IsNullOrEmpty(pd.sql)) qry.sql = pd.sql;
                    }
                    else
