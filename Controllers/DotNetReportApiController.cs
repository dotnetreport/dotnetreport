...
namespace ReportBuilder.Web.Controllers
{
    public class DotNetReportApiController : Controller
    {
        ...
        private DotNetReportSettings GetSettings()
        {
            ...
            settings.UserId = "Guest"; // You can pass your current authenticated user id here to track their reports and folders            
            settings.UserName = "";
            settings.CurrentUserRole = new List<string>(); // Populate your current authenticated user's roles

            settings.Users = new List<dynamic>() { "John", "Jane", "Guest" }; // Populate all your application's user, ex  { "Jane", "John" } or { new { id="1", text="Jane" }, new { id="2", text="John" }}
            settings.UserRoles = new List<string>() { "Admin", "Limited" }; // Populate all your application's user roles, ex  { "Admin", "Normal" }       
