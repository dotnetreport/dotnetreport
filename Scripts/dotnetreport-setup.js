...
var manageViewModel = function (options) {
	...
	self.emailQueries = new function () {
		...
		q.previewQuery = function (item) {
			ajaxcall({
				url: options.previewEmailListUrl + '?id=' + item.id,
				type: 'GET'
			}).done(function (x) {
				if (x.d) x = x.d;
				if (x.result) x = x.result;
...
var usersRolesViewModel = function (options, settings, previewData) {
	...
	self.loadSqlSources = function () {
		_.forEach(self.sqlSources, function (src) {
			ajaxcall({
				url: options.reportsApiUrl,
				data: { method: options.getEmailQueriesUrl, model: JSON.stringify({ includeGlobal: true, queryType: src.queryType }), userId: self.currentUserId || '' },
			}).done(function (x) {
				if (x.d) x = x.d;
				if (x.result) x = x.result;
				if (x.Result) x = x.Result;
	...
	self.deleteSqlSource = function (src) {
		bootbox.confirm('Are you sure you would like to delete the ' + ko.unwrap(src.title) + '?', function (r) {
			...
			ajaxcall({
				url: options.reportsApiUrl,
				data: { method: options.deleteEmailQueryUrl, model: JSON.stringify({ id: src.savedId() }), userId: self.currentUserId || '' }
			}).done(function () {
				src.savedId(0);
				src.sqlQuery('');
				src.rows([]);
	...
	var api = function (method, model) {
		...
		return ajaxcall({
			url: options.apiUrl,
			type: 'POST',
			data: JSON.stringify({ method: '/ReportApi/' + method, model: JSON.stringify(model), userId: self.currentUserId || '' })
		});
	};
	...
	self.sendPasswordSetup = function (u) {
		...
		ajaxcall({
			...
			type: 'POST',
			data: JSON.stringify({ email: u.email, userId: self.currentUserId || '' })
		}).done(function (r) {
			if (r.d) r = r.d;
